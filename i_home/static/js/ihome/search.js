...
$(document).ready(function () {
    ...
    $.get('/ihome/searchall/' + path, function (data) {
        // console.log(data);
        if (data.code == '200'){
            var alist_html = template('all_area_list', {alist:data.alist});
            $('.filter-area').html(alist_html);
            // console.log(data.new_hosue_list);
            var new_house_html = template('all_delorder_house', {new_house_list:data.hlist});
            $('.house-list').html(new_house_html)
        }else{
            alert('请至少选择时间和地区')
        }
    })
